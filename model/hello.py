...
def public_category(category=None):
    listProduct = []
    if (int(category) >= 10):
        pro_result = SelectDBWhere('product', "refcategory = '" + category + "' AND active = '1'")
    else:
        pro_result = SelectDBWhere('product', "refcategory in (SELECT id FROM category WHERE refparent = '" + category + "') AND active = '1'")
    for prd in pro_result:
        listProduct.append(Product(prd[0], prd[1], prd[2], prd[3], prd[4], prd[5], prd[6], prd[7], prd[8], prd[9]))
